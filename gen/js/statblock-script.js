...
function TryMarkdown() {
    ...
    let markdown = ['<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>', mon.name, '</title><link rel="shortcut icon" type="image/x-icon" href="./public/favicon.ico" /></head><body>'];
...
var FormFunctions = {
    ...
    ChangeCRForm: function () {
        if (mon.cr == "*") {
            ...
            $("#prof-bonus").show();
            $("#prof-bonus").html("(Бонус досвіду: +" + StringFunctions.RemoveHtmlTags(CrFunctions.GetProf()) + ")");
            $("#custom-cr").hide();
        }
    },
...
var CrFunctions = {
    ...
    GetString: function () {
        ...
        return mon.cr + " (" + data.crs[mon.cr].xp + " ПД)"
    }
}
